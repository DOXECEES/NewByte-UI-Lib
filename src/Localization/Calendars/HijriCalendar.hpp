...
namespace Localization
{
    ...
    public:
        ...
        // Дни недели (в исламской культуре первым днем считается воскресенье, 
        // но индекс в вашей системе остается прежним для совместимости)
        ...
        {
            // Используется стандартный 30-летний цикл (11 високосных лет)
            // Високосные годы: 2, 5, 7, 10, 13, 16, 18, 21, 24, 26, 29
        ...
        int32 getDaysInMonth(int32 year, int32 month) const noexcept override
        {
            ...
            // Нечетные месяцы - 30 дней, четные - 29.
            // В високосный год 12-й месяц имеет 30 дней.
        ...
        {
            int64 y = year;
            // Эпоха Хиджры: 16 июля 622 года н.э. = Fixed Day 227015
            return day                      // Дни в текущем месяце
                + (month - 1) * 29          // Базово 29 дней на месяц
                + month / 2                 // Добавляем 1 день для каждого нечетного месяца
                + (y - 1) * 354             // Базово 354 дня в году
                + (11 * y + 3) / 30         // Добавляем високосные дни из цикла
                + 227014;                   // Смещение относительно абсолютной эры
        }
        ...
        {
            // Перевод через абсолютные дни (Rata Die)
